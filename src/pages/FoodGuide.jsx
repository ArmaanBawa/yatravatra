    ...
    <div className="min-h-screen bg-background py-12">
      <div className="container">
        <div className="mb-8 flex items-center">
          ...
          <h1 className="text-4xl font-bold text-foreground text-center w-full">Delhi Food Guide</h1>
        </div>
        <p className="mb-12 text-center text-lg text-muted-foreground">
          Discover the best food spots and must-try dishes near Delhi's top attractions!
        </p>
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {delhiPlacesData.map((place) => (
            <div key={place.id} className="rounded-lg border bg-card p-6 shadow-sm">
              ...
              <h3 className="mb-2 text-lg font-bold text-foreground">Nearby Food Spots</h3>
              <ul className="mb-2 list-disc list-inside text-muted-foreground">
                {place.nearbyRestaurants
                  ? place.nearbyRestaurants.split(',').map((r, i) => (
                      <li key={i}>{r.trim()}</li>
                    ))
                  : <li>No food recommendations listed.</li>}
              </ul>
            </div>
          ))}
        </div>
